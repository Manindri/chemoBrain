import React, { useState, useEffect } from 'react'
import { Text, View, FlatList, Pressable, Image, ImageEditor } from 'react-native';
import { firebase } from "../firebase/config";
import { KeyboardAwareScrollView } from 'react-native-keyboard-aware-scroll-view';
import { useRoute } from '@react-navigation/native';
import { AntDesign } from '@expo/vector-icons';
import styles from './styles';
import { useNavigation } from '@react-navigation/native';
import HomeIcon from './HomeIcon';

export default function EventList() {

    const route = useRoute();
    const [events, setEvents] = useState([]);
    const eventRef = firebase.firestore().collection('events');

    const navigation = useNavigation();


    useEffect(async () => {
        eventRef
            // order by time of creating
            //.orderBy('createdAt', 'desc')
            // fetch todos in realtime
            .onSnapshot(
                querySnapshot => {
                    const events = []
                    // loop through the saved todos
                    querySnapshot.forEach((doc) => {
                        const event = doc.data()
                        event.id = doc.id,
 
                            events.push(event)
                    });
                    // set the todos to the state
                    setEvents(events)
                },
                error => {
                    // log any error
                    console.error(error);
                }
            )
    }, []);

    const deleteEvent = (event) => {
        // delete event from firestore db
        eventRef
            .doc(event.id)
            .delete()
            .then(() => {
                // show a successful alert
                alert("Deleted successfully");
            })
            .catch(error => {
                // show an error alert
                alert(error);
            })
    }


    return (
        <View style={styles.container}>
            <KeyboardAwareScrollView
                style={{ flex: 1, width: '100%' }}
                keyboardShouldPersistTaps="always">
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <AntDesign name="arrowleft" size={24} color="#fff" onPress={() => navigation.navigate('Add Event')} style={styles.backbtn} />
                    <Image
                        style={styles.logo}
                        // source={require('./colorLogo.png')}
                    />
                    <Text style={styles.listlogotext}>EVENT LIST</Text>
                </View>

                <View style={styles.ListFormRec}>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        {/* <AntDesign name="arrowleft" size={24} color="#fff" onPress={() => navigation.navigate('Add')} style={styles.backbtn} /> */}
                        <Image
                            style={styles.Listlogo}
                            source={require('./colorLogo.png')}
                        />
                        <Text style={styles.Listlogotext}>EVENT LIST</Text>
                    </View>
                </View>
                <FlatList
                    style={{ height: '100%' }}
                    data={events}
                    numColumns={1}
                    renderItem={({ item }) => (
                        <Pressable style={styles.listContainer} onPress={() => navigation.navigate('View Event', { itemId: item.id })}>
                            <View style={styles.innerListContainer}>
                                <Image
                                    style={styles.eventImage}
                                    source={require('./eventPeople.png')}
                                />
                                <View style={styles.listText}>
                                    <Text style={styles.listBoxText}>{item.eventName}</Text>
                                    <Text style={styles.listBoxText}>{item.eventDate}</Text>
                                </View>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <AntDesign
                                        name="edit"
                                        size={24}
                                        color="#48A3F3"
                                        onPress={() => navigation.navigate('Update Event', { itemId: item.id })}
                                        style={styles.listEditIcon} />

                                    <AntDesign
                                        name="delete"
                                        size={24}
                                        color="#48A3F3"
                                        onPress={() => deleteEvent(item)}
                                        style={styles.listDeleteIcon} />
                                </View>
                            </View>
                        </Pressable>
                    )}
                >
                </FlatList>
            </KeyboardAwareScrollView>
        </View>

    )

}

